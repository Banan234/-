...
let catalogCacheByCategory = null;
let catalogListCacheItems = null;
let catalogListCache = null;
let catalogListCacheByCategory = null;
...
function buildCatalogCategoryIndex(items) {
  ...
  for (const item of items) {
    const key = item.catalogCategorySlug;
    if (!key) continue;
...
function getConductorMaterial(product) {
  const decoded = Array.isArray(product?.cableDecoded?.decoded)
    ? product.cableDecoded.decoded
    : [];
...
function createCatalogListProduct(product) {
  ...
  assignPresent(listProduct, 'brand', product.manufacturer || product.catalogBrand);
  ...
  assignPresent(listProduct, 'catalogApplicationType', product.catalogApplicationType);
...
function buildCatalogListCache(items) {
  ...
  for (let i = 0; i < items.length; i += 1) {
    const key = items[i].catalogCategorySlug;
    if (!key) continue;
...
function ensureCatalogListCache(items) {
  if (items !== catalogCache) {
    return buildCatalogListCache(items);
  }

  if (catalogListCacheItems !== items || !catalogListCache || !catalogListCacheByCategory) {
    const next = buildCatalogListCache(items);
    catalogListCacheItems = items;
    catalogListCache = next.listItems;
    catalogListCacheByCategory = next.index;
  }

  return {
    index: catalogListCacheByCategory,
    listItems: catalogListCache,
  };
}

export function getCatalogProductListItems(items = catalogCache) {
  if (!items) return [];
  return ensureCatalogListCache(items).listItems;
}

export function getCatalogProductListItemsByCategory(
  categorySlug,
  items = catalogCache
) {
  if (!items) return [];
  ...
  return ensureCatalogListCache(items).index.get(categorySlug) || [];
}

export function getCatalogProductsByCategory(categorySlug, items = catalogCache) {
  if (!items) return [];
  if (!categorySlug) return items;

  const index =
    items === catalogCache
      ? catalogCacheByCategory || (catalogCacheByCategory = buildCatalogCategoryIndex(items))
...
export async function loadCatalogProducts() {
  ...
  catalogCacheByCategory = buildCatalogCategoryIndex(catalogCache);
