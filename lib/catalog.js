...
const COMMUNICATION_CABLE_IMAGE =
  '/category-placeholders/communication-cable.svg';
...
function findImageByMatcher(rawProduct, state) {
  ...
  const match = state.imageMatchers.find((m) => {
    ...
    if (
      m.crossSection != null &&
      m.crossSection !== Number(rawProduct.crossSection)
    ) {
      return false;
    }
    if (m.voltage != null && m.voltage !== Number(rawProduct.voltage))
      return false;
    return true;
  });
...
export function createCatalogStore({
  ...
  async function loadCatalogProducts() {
    ...
    if (
      state.catalogCache &&
      stat.mtimeMs === state.catalogCacheMtime &&
