// Файл проверяет серверную совместимость и правила общей валидации заявок.
...
describe('isValidQuoteRequest', () => {
  ...
  it('требует корректный российский телефон', () => {
    ...
    for (const phone of ['3333333333', '1234567890', '+7 999 999-99-99']) {
      expect(
        isValidQuoteRequest({
  ...
  it('требует согласие на обработку данных', () => {
    expect(
      isValidQuoteRequest({
        customer: { ...baseCustomer, consent: false },
        items: [baseItem],
      })
    ).toBe(false);
  });
...
describe('isValidRussianPhone', () => {
  ...
  it('отклоняет короткие, повторяющиеся и последовательные номера', () => {
    ...
    expect(isValidRussianPhone('3333333333')).toBe(false);
